...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200):
  ...
  for i in [0,100,500,1000,1500,2000]:
     ...
     neigh_i = neigh[1][ii]
     subv, subn, subf = extract_patch(mesh, neigh_i)
...
def output_patch_coords(subv, subf, subn, i, neigh_i, theta, rho):
  ...
  charge = np.random.rand(len(neigh_i))
...
def output_patch_coords2(subv, subf, subn, i, neigh_i, theta, rho, feats, outDir):
  """
      For debugging purposes, save a patch to visualize it.
  """
  mesh = pymesh.form_mesh(subv, subf)
  ...
  featNames = ['si', 'ddc', 'hbond', 'charge', 'hphob']
  for fi, fName in enumerate(featNames):
    mesh.add_attribute(fName)
    mesh.set_attribute(fName, np.reshape(feats[:,fi], feats.shape[0]))

  if not os.path.exists(outDir):
    os.mkdir(outDir)
  pymesh.save_mesh(outDir+'/v{}.ply'.format(i), mesh, *mesh.get_attribute_names(),
                     use_float=True, ascii=True)
...
def outputPocketPatches(mesh, pocket_is, neighs_is, theta, rho, feats, outDir):
  for i in pocket_is:
    subv, subn, subf = extract_patch(mesh, neighs_is[i])
    output_patch_coords2(subv, subf, subn, i, neighs_is, theta[i], rho[i, :], feats[i], outDir)
...
for i, pdb in enumerate(all_pdbs):
        ...
        if True:
            ...
            inp = os.path.join(ligand_coord_dir, "{}_ligand_coords.npy".format(pdb.split("_")[0]))
        ...
        for j, structure_ligand in enumerate(all_ligand_types):
            ...
            pocket_indices = pocket_points_flatten
            patches_feat = input_feat[pocket_points_flatten]
            patches_rho = rho_wrt_center[pocket_points_flatten]
            patches_theta = theta_wrt_center[pocket_points_flatten]

        fields = pdb.split('_')
        ply_file = masif_opts['ply_file_template'].format(fields[0], fields[1])
        mesh = pymesh.load_mesh(ply_file)
        neighFile = precom_dir + pdb + '/p1_list_indices.npy'
        neigh_i = np.load(neighFile, encoding='latin1', allow_pickle=True)

        #compute_polar_coordinates(mesh)
        pocket_neighs_i = neigh_i[pocket_points_flatten]
        #outputPocketPatches(mesh, pocket_is, neighs_is, theta, rho)
        outDir = "data_preparation/pocket_patches/"+pdb
        outputPocketPatches(mesh, pocket_points_flatten, neigh_i,
                            theta_wrt_center, rho_wrt_center, input_feat, outDir)
